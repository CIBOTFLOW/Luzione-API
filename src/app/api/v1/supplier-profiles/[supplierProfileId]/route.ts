...
export async function GET(request: Request, context: { params: Promise<{ supplierProfileId: string }> }) {
  ...
  try {
    ...
    const supplierProfileId = supplierProfileRouteId((await context.params).supplierProfileId, "supplierProfileId");
    identity = bindAuthenticatedRequestIdentity(identity, actor, { authorityClass: "A0", capability: "supplier.profile.read", purpose: "read-supplier-profile", sourceVersionRefs: [SUPPLIER_PROFILE_CONTRACT_VERSION] });
    const observedAt = new Date().toISOString();
    const readback = await new SeedSupplierIdentityStore().readByIdWithTimeline(actor, supplierProfileId, observedAt);
    if (!readback) return apiResponse({ ok: false, code: "SUPPLIER_PROFILE_NOT_FOUND", message: "Supplier Profile not found for this tenant." }, { requestIdentity: identity, status: 404 });
