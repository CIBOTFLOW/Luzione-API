...
export async function GET(request: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    const actor = await requireServiceActor(request.headers, "specification_schedule.read");
    identity = bindAuthenticatedRequestIdentity(identity, actor, {
      authorityClass: "A0",
      capability: "specification_schedule.read",
      purpose: "read-canonical-project-specification-schedule",
