...
class DenyingAdmissionStore extends FakeStore {
  override async requireStage5Admission(): Promise<never> {
    this.admissionChecks += 1;
    throw new SultanAgentGatewayError(
      "STAGE5_ADMISSION_DENIED",
      "The exact immutable Stage 5 admission did not validate.",
      403,
    );
  }
}
...
test("no Sultan read or command preparation can bypass exact Stage 5 admission", async () => {
  ...
  const noteCall = call("luzione.note.append", {
    campaignId: "sultan-campaign-pilot-001",
    note: "Synthetic test note; no customer data.",
  });
  await assert.rejects(
    () => service.invoke({ actor: actor(), call: noteCall }),
    (error: unknown) => error instanceof SultanAgentGatewayError && error.code === "STAGE5_ADMISSION_DENIED",
  );
  assert.equal(store.admissionChecks, 2);
  assert.equal(store.preparations, 0);
...
test("command parser accepts only the versioned execution envelope", () => {
  ...
  assert.throws(
    () => parseCommandExecutionEnvelope({ execution: {
      contractVersion: LUZIONE_SULTAN_COMMAND_EXECUTION_V1,
      reservationId: "sultan-reservation-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      commandHash: "a".repeat(64),
    } }),
    (error: unknown) => error instanceof SultanAgentGatewayError && error.code === "HUMAN_APPROVAL_REQUIRED",
  );
});
