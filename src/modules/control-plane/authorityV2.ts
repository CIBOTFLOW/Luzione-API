...
export function evaluateAuthorityV2(input: EvaluationInput): AuthorityDecision {
  ...
  if (envelope.authorityClass === "A2") {
    const internalChange = !capability.providerEffect
      && capability.operationKind === "INTERNAL";
    const externalEffect = capability.providerEffect
      && capability.operationKind === "EXTERNAL";
    if (!internalChange && !externalEffect) {
      return decision(
        input,
        false,
        "OPERATION_CLASS_MISMATCH",
        "A2 is reserved for bounded reversible internal changes or external effects.",
      );
    }
    if (!action.readbackPlanned) {
      return decision(
        ...
        "A2 requires authoritative source readback before completion.",
      );
    }
    ...
    return decision(
      ...
      "ALLOW_A2",
      internalChange
        ? "The bounded reversible internal change satisfies readback and recovery requirements."
        : "The bounded reversible external effect satisfies readback and recovery requirements.",
    );
  }
