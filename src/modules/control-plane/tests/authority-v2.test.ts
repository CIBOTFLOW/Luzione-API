...
test("A2 supports a reversible internal learning change without provider authority", () => {
  const internalCapability = capability({
    authorityClass: "A2",
    capability: "learning.candidate.promote",
  ...
  const internalEnvelope = {
    ...envelope,
    authorityClass: "A2" as const,
    capability: internalCapability.capability,
  };
  const internalAction = {
    ...action,
    compensationPlanRef: "learning-rollback:exact-last-known-good",
    provider: internalCapability.provider,
  };

  const allowed = evaluateAuthorityV2({
    action: internalAction,
    capability: internalCapability,
    envelope: internalEnvelope,
    now: "2026-08-28T00:00:00Z",
  });
  assert.equal(allowed.code, "ALLOW_A2");
  assert.equal(allowed.externalEffectAuthorized, false);
  assert.equal(
    evaluateAuthorityV2({
      action: { ...internalAction, readbackPlanned: false },
      capability: internalCapability,
      envelope: internalEnvelope,
      now: "2026-08-28T00:00:00Z",
    }).code,
