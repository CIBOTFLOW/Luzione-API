...
export const securityControlRegistry: readonly SecurityControl[] = Object.freeze([
  ...
  },
  {
    ...
    evidenceRefs: ["src/modules/security-posture/rlsPosture.ts", "src/modules/security-posture/tests/rls-posture.test.ts", "supabase/migrations/20260831090000_api_pc_013_least_privilege_roles_rls.sql", "architecture/production-convergence/API_PC_013_OWNERSHIP_MANIFEST.json"],
    evidenceState: "IMPLEMENTED_LOCAL",
    invariant: "Sensitive server-only and admitted production-convergence relations enforce the required RLS posture; runtime roles are non-login/non-owner/non-bypass, browser and legacy service roles have no direct authority, and the provider worker stays inside its exact delivery scope.",
    owner: "Luzione database and security owner",
    probe: "Catalog role/grant/forced-RLS posture plus active anon/authenticated denials and disposable missing-tenant/cross-tenant/worker-scope probes.",
