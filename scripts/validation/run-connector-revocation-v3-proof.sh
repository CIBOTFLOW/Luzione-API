...
host_port="${2:-5432}"
suffix="$$"
database="luzione_connector_revocation_v3_${suffix}"
role="connector_revocation_v3_proof_${suffix}"
password="connector_revocation_v3_disposable_${suffix}"
...
NODE_PATH=scripts/validation/node-stubs \
DATABASE_URL="postgres://${role}:${password}@127.0.0.1:${host_port}/${database}" \
node --import tsx scripts/validation/connector-revocation-v3-postgres-proof.ts

set +e
populated_reverse_output="$(docker exec -i "${container_name}" psql -q -v ON_ERROR_STOP=1 -U postgres -d "${database}" < scripts/validation/rollback-connector-revocation-locator-redaction.sql 2>&1)"
populated_reverse_status=$?
set -e
test "${populated_reverse_status}" -ne 0
case "${populated_reverse_output}" in
  *"reverse blocked before DDL: ConnectorRevocationReceipt/v3 or locator-redaction evidence exists"*) ;;
  *) echo "${populated_reverse_output}" >&2; exit 1 ;;
esac
post_refusal_columns="$(docker exec "${container_name}" psql -At -v ON_ERROR_STOP=1 -U postgres -d "${database}" -c "select count(*) from information_schema.columns where table_schema='public' and table_name='connector_revocation_receipts' and column_name in ('binding_readback_contract_version','binding_head_digest','credential_content_digest','public_readback_id','public_readback_digest','request_raw_digest','canonical_readback')")"
test "${post_refusal_columns}" = "7"

echo "empty_reverse_columns=${reversed_columns}"
echo "empty_reapply_columns=${reapplied_columns}"
echo "populated_reverse=blocked_before_ddl"
echo "post_refusal_columns=${post_refusal_columns}"
