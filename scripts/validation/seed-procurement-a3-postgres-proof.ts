...
function evidenceCommand(projectId: string, projectVersion: string, input: { commandId: string; digest: string; source: string }) {
  return parseSeedProcurementCommand({ ...common(input.commandId, "evidence_artifact.register"), projectRef: { projectId, projectVersion }, artifact: { capturedAt: requestedAt, confidence: 0.95, contentDigest: input.digest, kind: "UPLOAD", mimeType: "text/csv", promptInjectionState: "NOT_ASSESSED", provider: "OPERATOR_UPLOAD", sourceRecordRef: input.source, storageRef: `private-object:${input.source}` } }) as EvidenceArtifactRegisterCommand;
}
...
async function main() {
  try {
    ...
    const po = parseSeedProcurementCommand({ ...common("po-a3", "purchase_order.create_draft"), bidComparisonId: "bid-a3", expectedVersion: "bid-comparison:bid-a3:v2", lineRefs: [{ objectId: specificationLineId, objectType: "SPECIFICATION_LINE", ownerProject: "LUZIONE_PROJECT", version: specificationLineVersion }], projectId, projectVersion, proposalVersion: "proposal:unverified:v1", proposalVersionId: "proposal-unverified", selectionDecisionId: "selection-a3", selectionDecisionVersion: "bid-comparison:bid-a3:v2" });
    ...
    assert.equal(parsedGraph.productSources.length, 1);
    ...
    console.log(JSON.stringify({ atomicityFaultRollback: faultCounts[0], canonicalSupplierIdentity: "ACCOUNT_PRESENT_ELIGIBILITY_UNVERIFIED", concurrency: { exactReplay: "one receipt", supplierQuotes: "both denied before write" }, dependencyHeldRows: 0, freshInstanceReadback: true, ledger: ledger[0], objectiveScore: objectiveScore(fit), proposalDependency: "CANONICAL_READER_UNAVAILABLE", tenantIsolation: true }, null, 2));
