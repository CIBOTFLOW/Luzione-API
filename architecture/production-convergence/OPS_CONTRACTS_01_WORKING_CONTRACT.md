...
Queued beside it under controller `b3dbea8` is `CONNECTOR-FOUNDATION-01`: one default-off `POST /api/v1/connectors/sync-validations` boundary reusing `ConnectorBinding/v1`, `SyncReceipt/v1`, service actor, P110 idempotency/outbox, provider runtime, and `SandboxEchoProviderAdapter`, with tenant/key reservation, server-recomputed payload digest, NO_EFFECT authority, and API-exclusive receipt/finality semantics. It adds no schema or migration and is likewise not started here.
